...
class MovieDataModule(LightningDataModule):
    ...
    def setup(self, stage):
        ...
        for i in range(len(data)):
            user_seq_dict[user_map[data.loc[i, "user"]]].append([item_map[data.loc[i, "item"]], data.loc[i, "time"]])
    ...
    def train_dataloader(self):
        '''making the the data retrieval code from original more modular but using the same logic
        change: original allows for repetitions of users in batch while we go through all user in shuffled fashion as 
        dataloaders do.
        '''
        ...
        for user in self.user_seq_train:
            ...
            if len(self.user_seq_train[user][0]) > 0:
                pos_len = len(self.user_seq_train[user][1])
                if pos_len > self.config.pos_num:
                    for i in range(self.config.pos_num):
                        self.user_pos_train.append((user, -1))
                else:
                    pos_idx = np.arange(pos_len)
                    for pos_i in pos_idx:
                        self.user_pos_train.append((user, pos_i))

        print(len(self.user_pos_train))

        train_dataset = TrainDataset(self.user_pos_train, self.user_seq_train, self.item_n, self.config.max_seq_len, self.config.negs_ppos)
    ...
    def val_dataloader(self):
        ...
        for user in self.user_seq_valid:
            ...
            if len(self.user_seq_valid[user][0]) > 0:
                ...
                pos_len = len(self.user_seq_valid[user][1])

                for pos_i in range(pos_len):
                    self.user_pos_val.append((user, pos_i))
        ...
        for user in self.user_seq_test:
            ...
            if self.p90:
                ...
                if len(self.user_seq_test[user][0]) > 0:
                    ...
                    pos_len = len(self.user_seq_test[user][1])

                    self.user_pos_test.append((user, 0))
                ...
                if len(self.user_seq_test[user][0]) > 0:
                    ...
                    for pos_i in range(pos_len):
                        self.user_pos_test.append((user, pos_i))

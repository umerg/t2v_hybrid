...
class LightT2VSAS(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        log_seqs, time_seqs, pos_seqs, neg_seqs = batch["input_seq"], batch["time_seq"], batch["pos_seq"], batch["neg_seq"]
        ...
        if self.global_step % self.config.cycling_ratio == 0:

            pos_embs = self.model.item_emb(pos_seqs).to(self.device) 
            neg_embs = self.model.item_emb(neg_seqs).to(self.device)

            #print("pos embedded: ", pos_embs)
            #print("neg embedded: ", neg_embs)

            pos_logits = (log_feats * pos_embs).sum(dim=-1)
            neg_logits = (log_feats * neg_embs).sum(dim=-1)

            #print("pos scores: ", pos_logits)
            #print("neg scores: ", neg_logits)

            pos_labels = torch.ones(pos_logits.shape).to(pos_logits)
            neg_labels = torch.zeros(neg_logits.shape).to(neg_logits) #device + float for BCE loss
            
            #print("labels: ", pos_labels)
            #print("labels shape: ", pos_labels.shape)

            idx = torch.where(pos_seqs != 0)  #Torch or NP 

            #print("selected indexes: ", idx)
            #print("selected pos: ", pos_logits[idx])
            #print("selected neg: ", neg_logits[idx])

            loss = self.loss_fn(pos_logits[idx], pos_labels[idx])
            loss += self.loss_fn(neg_logits[idx], neg_labels[idx]) 

        else:
            
            final_feat = log_feats[:, -1, :]
            ##print("Forward ended")
            samples = torch.hstack([pos_seqs[:, -1].unsqueeze(-1), neg_seqs])
        ...
        return loss # For backprop #ADD ACCURACY
